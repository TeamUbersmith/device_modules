...
class dm_mydevicemodule extends device_module
{
	...
	function summary($request = array())
	{
		...
		$output .= 'Configured username is: ' . $username. '<br />';
	...
	{
		/**
		 * This code creates a bogus monitor that will probably fail most of the time.
		 */
		$value = rand(0, $monitor['extra']['min']) + 5;
		
		if (isset($monitor['extra']['min']) && strlen($monitor['extra']['min']) > 0) {
			if ($value < $monitor['extra']['min']) {
				return PEAR::raiseError(sprintf(uber_i18n('Value (%1$s) is less than minimum (%2$s)'),$value,$monitor['extra']['min']),1);
			}
		}
		
		if (isset($monitor['extra']['max']) && strlen($monitor['extra']['max']) > 0) {
			if ($value > $monitor['extra']['max']) {
				return PEAR::raiseError(sprintf(uber_i18n('Value (%1$s) is greater than maximum (%2$s)'),$value,$monitor['extra']['max']),1);
			}
	 ...
	 *
	 * If your device module is going to have a monitor, this function will return
	 * an array of the possible configuration options. For example, if you want
	 * to check if a value returned by your module is between two other values,
	 * you could set a minimum and maximum value, and use them in the 
	 * monitor() code above. 
	 *
	 * This function is optional.
	 * 
